<script>
  ...
  const drop = (colindex) => {
    let turn = 'x'
    let x = rows.flatMap(row => row).filter((value) => value == 'x').length
    let o = rows.flatMap(row => row).filter((value) => value == 'o').length
    if (x > o) {
      turn = 'o'
    }
...
<div class="grid grid-cols-7 gap-2 p-2">
  {#each rows as row, rowindex (rowindex)}
    {#each row as col, colindex (`${rowindex}-${colindex}`)}
      <button class="rounded-full w-16 h-16 {col == 'x' ? 'bg-red-500' : col ? 'bg-yellow-300' : 'border border-2'}" on:click={() => {
        drop(colindex)
      }}>
