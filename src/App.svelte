...
<div class="p-2">
	<button
		class="text-orange-300 cursor-pointer hover:text-orange-500"
		title="Reset"
		onclick={() => {
			restart();
		}}
...
<div class="p-2 min-w-fit mx-auto">
	<table class="">
		<tbody>
			{#each rows as row, rowindex (rowindex)}
				<tr>
					{#each row as col, colindex (`${rowindex}-${colindex}`)}
						<td>
							<button
								class="rounded-full w-16 h-16 cursor-pointer hover:border-orange-300 {col ==
								'red'
									? 'bg-red-500'
									: col
										? 'bg-yellow-300'
